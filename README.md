...
| `scripts/check_lesson_scope.py` | Lesson scope duplicate validator for the done pre-commit audit: flags a full rule body near-verbatim duplicated between a project lessons corpus and the company guidelines master (exit 0 clean/cold start, 1 duplicate, 2 usage/IO error). Deployed at runtime as a per-file symlink under ~/.ai-playbook/scripts/; done warns and continues when absent. |
