...
| `maintenance` | `agents/skills/maintenance/SKILL.md` | Unattended scheduler turn that surveys backlog, plans, and quota, then schedules at most one child session. | Decision order: execute the best available plan (digest-intact preferred), then author for the highest-priority uncovered backlog item, then no-op. Single child lane: at most one child in flight at a time (any kind). Failure cap: three consecutive no-progress child outcomes stop scheduling until a human clears the alert. Runtime overlay: `agents/skills/maintenance/zcode.md`. |
