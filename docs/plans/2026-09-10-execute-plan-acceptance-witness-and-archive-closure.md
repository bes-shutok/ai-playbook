...
- **Discriminating assertion**: an assertion that fails when the required behavior is absent or wrong, not a test class or method name, a test count, a status code, a checkbox, or a green-build claim.
...
- **Completion-evidence contract**: the plans-skill Evaluation Criteria rule and its not-evidence taxonomy sentence; peers reference it by this name.
...
- assume the activation fixture under `scripts/testdata/execute-plan/activation/` is byte-verified per role by `scripts/test_runtime_capabilities.py` (the activation package bytes check); the role map is closed (skill, driver, registry, adapter); this plan edits the sources of the skill and driver roles, so Tasks 1 and 2 sync those two fixture copies, while the registry and adapter roles stay byte-identical and untouched; the skill role had additionally drifted before this plan started; basis: the `activation.json` role map, the byte comparison in `scripts/test_runtime_capabilities.py`, and the suite failure reproduced and the repair verified at authoring time (2026-09-10).
...
- `scripts/testdata/execute-plan/activation/loaded/execute-plan/SKILL.md` *(fixture copy of the execute-plan SKILL.md, byte-verified by `scripts/test_runtime_capabilities.py`; synced by Tasks 1 and 2)*
- `scripts/testdata/execute-plan/activation/loaded/execute-plan/runtime/execute_plan_runtime.py` *(fixture copy of the driver, byte-verified by `scripts/test_runtime_capabilities.py`; synced by Task 1)*
...
- `scripts/testdata/execute-plan/activation/**` except the skill-role and driver-role copies synced in Tasks 1 and 2; reason: the registry and adapter roles are byte-identical probe fixtures whose sources this plan does not touch, and their probes stay green via the capabilities and codex suites.
...
grep -q "the completion gate fails up front" agents/skills/execute-plan/SKILL.md || { echo "missing: Phase 2 ceiling routing"; exit 1; }
grep -q "consolidate or split acceptance criteria before execution" agents/skills/execute-plan/runtime-contract.md || { echo "missing: runtime-contract ceiling routing"; exit 1; }
...
- `scripts/testdata/execute-plan/activation/loaded/execute-plan/SKILL.md` *(fixture sync)*
- `scripts/testdata/execute-plan/activation/loaded/execute-plan/runtime/execute_plan_runtime.py` *(fixture sync)*

The driver change and the prose that drives the new argument land in this one commit on purpose: a driver gate without the orchestrator prose would lock any run reaching Phase 5 into a blocked loop until the next commit. Residual window: the Phase 2 completion gate that produces the record lands in Task 2, so a run reaching Phase 5 inside this window constructs the record per the runtime-contract paragraph and the Phase 5 derivation rule (one criterion and artifact pair per acceptance criterion, each derivable from a named artifact) and re-issues the terminal call; a run of a plan whose criteria name no observable artifacts stops and reports a hard gate instead of improvising pairs. That stop follows the same exit state as the Phase 2 routing stop, and because the archive move was premature in that window, the retrofit un-archives the plan (`git mv` back under the active plans path), revises it there, and the re-reviewed run re-archives on passing.
...
- [ ] `ExecutePlanRuntimeTest#test_terminal_rejects_malformed_acceptance_evidence`; given `acceptance_evidence` that is an empty list, a non-list, a pair missing `criterion`, a pair with a whitespace-only `artifact`, a pair whose criterion is a non-string value, a 65-pair list, and a pair whose artifact exceeds `MAX_EVIDENCE_ITEM_BYTES` (one case each), expects a blocked `done-pending` outcome every time and `workflow_state` still `active`
- [ ] `ExecutePlanRuntimeTest#test_terminal_receipt_carries_acceptance_evidence`; given a complete manifest and `acceptance_evidence` of one pair carrying an extra key beyond criterion and artifact, expects a success outcome, `terminal_receipt.acceptance_evidence` equal to exactly the normalized criterion/artifact pair (extra keys dropped), and `terminal_result()` returning `acceptance_evidence` alongside `workflow_state: complete`
- [ ] `ExecutePlanRuntimeTest#test_terminal_result_over_legacy_complete_manifest`; given a manifest hand-written complete with a `terminal_receipt` that lacks `acceptance_evidence`, expects `terminal_result()` returning `None` and no `terminal-reissue` history event; after `mark_terminal` is re-issued with the full record (idempotent overwrite), expects a success outcome, `terminal_result()` returning the complete receipt, and a `terminal-reissue` history event whose recorded digest the test recomputes with exactly the pinned recipe from the Implement bullet
- [ ] Update the success-path call sites in `scripts/test_execute_plan_runtime.py` for the required record: the `mark_terminal` call in `test_driver_entrypoint_owns_transitions_across_reload`, the second (success) call in `test_active_manifest_suppresses_terminal_result`, and the `cli("terminal", ...)` payload in `test_cli_drives_claim_checkpoint_done_and_terminal` (add `acceptance_evidence` to the `--input` JSON). The production-side sites (module selftest, CLI dispatch) are Implement-bullet edits, so the RED run below executes against untouched production code. The two no-args blocked-path calls (the first call in `test_active_manifest_suppresses_terminal_result` and the call in the `conversational-permission-loop` replay handler) stay unchanged: they are blocked by the other required fields before evidence is checked
- [ ] Run → expect RED: `python3 scripts/test_execute_plan_runtime.py` fails all four new tests plus the two direct keyword call-site tests in `test_driver_entrypoint_owns_transitions_across_reload` and `test_active_manifest_suppresses_terminal_result` (`TypeError` on the unexpected keyword, plus blocked-versus-success and missing-receipt assertion failures); the CLI payload test in `test_cli_drives_claim_checkpoint_done_and_terminal` stays green at RED because the untouched dispatch drops unknown payload keys, and its discriminating power begins once the dispatch forwards the field in the Implement step; the remaining tests stay green
- [ ] Implement in `scripts/execute_plan_runtime.py`: one module-level validator that accepts only a non-empty list of mappings each carrying a criterion and an artifact that are strings, non-blank after stripping whitespace, and whose stripped string length is within the existing `MAX_EVIDENCE_ITEM_BYTES` bound (character semantics, matching the existing bounded_evidence helper); non-string values reject with the same blocked outcome. The pair list is capped at 64 entries (oversized input returns the same blocked outcome); normalization keeps exactly the `criterion` and `artifact` keys per pair (extra keys are dropped); both `mark_terminal` and `terminal_result` call that same validator (the read-time re-check is deliberate tamper detection between write and read); `mark_terminal` returns the blocked `done-pending` outcome with evidence `acceptance evidence is required before terminal state` when validation fails, stores the normalized pairs in `terminal_receipt.acceptance_evidence`, and on a manifest whose `workflow_state` is already complete appends a history event `{"event": "terminal-reissue", "prior_pair_count": <length of the prior receipt's acceptance_evidence list, 0 when the receipt or its list is absent>, "prior_receipt_digest": <sha256 of json.dumps(prior_receipt, sort_keys=True, separators=(",", ":")), with prior_receipt as JSON null when the receipt key is absent>}` so an overwrite is auditable down to what changed; `terminal_result` returns the pairs on success; the CLI terminal dispatch passes `payload.get("acceptance_evidence")`; the module selftest passes a complete record
- [ ] Document in `agents/skills/execute-plan/runtime-contract.md`, directly after the CLI operation table: one short paragraph stating that the terminal operation requires the acceptance-evidence record (one criterion and artifact pair per plan acceptance criterion; at most 64 pairs, and a plan whose acceptance criteria exceed 64 fails the Phase 2 completion gate up front and routes through the plans skill and a fresh plan review to consolidate or split acceptance criteria before execution, so the ceiling is never first discovered at the terminal call), that the driver keeps `workflow_state` active and returns a blocked `done-pending` result when the record is missing or incomplete, that the machine gate verifies presence and shape only (completeness per criterion and discrimination are enforced by the Phase 2 prose gate and the review lenses), that a complete manifest whose receipt predates the record returns no receipt until the terminal operation is re-issued with the full record, that a re-issue on an already-complete manifest appends a `terminal-reissue` history event recording the prior pair count and prior receipt digest while a recovery record reconstructed for a legacy run is retroactive and not machine-verified, and that when the Phase 5 stop fires after a premature archive the recovery un-archives the plan via `git mv` back to the active plans path, reconciles the premature ownership-registry row, and routes through the plans skill retrofit
- [ ] In `agents/skills/execute-plan/SKILL.md` Phase 5 Terminal receipt paragraph: change the call to pass the record, namely "with the archived-plan path, the last commit SHA, the Phase 5 success checklist, and the acceptance-evidence record"; add the sentences "The driver returns no terminal receipt while the acceptance-evidence record is missing or incomplete." and "Each pair must be derivable from a plan acceptance criterion's named artifact; an orchestrator that cannot derive a pair must stop and report a hard gate rather than construct one."; and extend the mirrored fields list with the acceptance-evidence record
...
- [ ] Sync the activation fixture copies for every role whose source this task edits, in the same commit: copy `agents/skills/execute-plan/SKILL.md` over `scripts/testdata/execute-plan/activation/loaded/execute-plan/SKILL.md` and copy `scripts/execute_plan_runtime.py` over `scripts/testdata/execute-plan/activation/loaded/execute-plan/runtime/execute_plan_runtime.py` (both roles are byte-verified by `scripts/test_runtime_capabilities.py`; the skill role had drifted before this plan started; this repair makes the capabilities suite green at the Task 1 gate)
...
Ordering note: the plans-side contract text that Phase 2 item 3 and the Integration Points line reference lands in Task 3, one commit later; until then item 3's inlined artifact-kind enumeration is the operative definition and the plans pointer becomes live at Task 3.
...
4. If, for a plan authored under the completion-evidence contract, any criterion names no artifact, or its artifact is missing, unchecked, or non-discriminating, the completion gate fails: treat it as a new fix iteration (the gap is in the implementation or a review miss, not the plan text). Phase 3 and Phase 4 archive stay blocked until the record is complete. Never mark a criterion complete without its artifact's fresh output. If the plan's acceptance criteria exceed 64 (the machine record ceiling), the completion gate fails up front when item 3's count runs: discard any mapping already drafted for `manifest.md`, stop and route the plan through the plans skill and a fresh plan review to consolidate or split acceptance criteria before execution. For a plan authored before the authoring-side binding whose criteria name no artifacts, stop and route the plan through the plans skill and a fresh plan review to retrofit artifacts; do not burn fix iterations on a gap that lives in the plan text. Treat either stop as a hard gate requiring user direction: record it in `manifest.md`, leave the machine manifest preserved, make no further driver calls, and re-enter only after the retrofit via the skill resume procedure (readiness gate against the re-reviewed plan, manifest refresh, then the Phase 2 completion gate); do not call the driver resume operation, which has no blocked task to resume on this manifest.
...
- [ ] Add `ExecutePlanRuntimeTest#test_completion_gate_prose_is_pinned`; given the repository skill files resolved from the test module location (`Path(__file__).resolve().parents[1]`, matching the existing runtime-neutral scan), expects each pinned phrase to be present exactly in its owning file, and a missing or unreadable file fails the test (no skip): in `agents/skills/execute-plan/SKILL.md` the phrases "Run the completion gate: enumerate every acceptance criterion", "archive only after the Phase 2 completion gate passed", "the Phase 5 success checklist, and the acceptance-evidence record", "the Phase 5 checklist, and the acceptance-evidence record", "must stop and report a hard gate rather than construct", and "the completion gate fails up front"; in `agents/skills/execute-plan/runtime-contract.md` the phrases "requires the acceptance-evidence record" and "consolidate or split acceptance criteria before execution", so post-archive paraphrase drift fails a suite instead of nothing
- [ ] Sync the activation fixture skill copy in the same commit: copy `agents/skills/execute-plan/SKILL.md` over `scripts/testdata/execute-plan/activation/loaded/execute-plan/SKILL.md` (it must carry this task's Phase 2 and Phase 4 edits when the capabilities suite next runs)
...
- [ ] Confirm the two-tier Review Scope still matches the branch diff via `git diff --name-only <BASE_BRANCH>...HEAD`: the output contains exactly the Review Scope explicit must-fix and test files, plus this plan file itself (mutated by per-task checkbox marking and the Phase 4 archive move), plus the backlog origin file `docs/history/backlog/2026-09-10-execute-plan-acceptance-witness-and-archive-closure.md` (mutated by the Status: done edit and the same-commit Phase 4 move, so the diff may list either or both of its paths); an unlisted path is a finding only when it is not causally plan-related under the Review Scope extension tier, and a path that implements or repairs a plan task is recorded as expected with the causal link noted; when the run proceeded on the default branch after a declined branch creation, replace the branch diff with `git log --name-only <session_start_commit>..HEAD`
