...
- [ ] Run → expect GREEN: the discovery command passes including the three new witnesses.
- [ ] Commit: `feat: launch-record snapshot with drift detection`
...
- [ ] Run → expect RED: `python3 -m unittest discover -s scripts -p 'test_execute_plan_runtime*.py'` for both new witnesses (the `create` operation and the directory-valued allowed-path rejection).
- [ ] Implement the `create` operation (the dispatch handler is a named symbol containing `_operation_create`, which the Validation pin keys on, since the bare string `"create"` cannot discriminate) and land both doc edits.
- [ ] Run → expect GREEN: the discovery command passes; `expect_match '_operation_create' scripts/execute_plan_runtime.py` holds.
...
- [ ] Dead `read_label` is deleted; the used argument loops give a clear error on a missing `--label` value and reject an empty label; the selftest gains one case per stance (missing value, empty label) asserting the specific error.
- [ ] `try_acquire`'s two near-identical acquisition bodies collapse into one helper used by both call sites; the selftest drives one contended and one uncontended acquisition per call site through the shared helper; `bash scripts/done-lock.sh selftest` stays green.
...
- [ ] Commit: `chore: close execute-plan runtime residuals origins and archive plan`
