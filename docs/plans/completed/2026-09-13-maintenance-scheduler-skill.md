...
Backlog origin: `docs/history/backlog/completed/2026-09-13-maintenance-scheduler-skill.md` (scope of record; its "Verified feasibility facts" section is ground truth and its "Open decisions for the plan author" are resolved in `## Assumptions` with receipts).
...
grep -qF 'name: maintenance' "$S" || { echo 'missing pin: frontmatter name'; exit 1; }
grep -qF 'plus 6 hours' "$S" || { echo 'missing pin: outcome horizon'; exit 1; }
grep -qF 'last 20 entries' "$S" || { echo 'missing pin: children retention'; exit 1; }
grep -qF 'consecutive_turn_errors' "$S" || { echo 'missing pin: turn error counter key'; exit 1; }
grep -qF 'never to D2' "$S" || { echo 'missing pin: G1 trip resolution'; exit 1; }
grep -qF 'at least 30 minutes out' "$S" || { echo 'missing pin: child fire offset'; exit 1; }
grep -qF 'temp file plus atomic replace' "$S" || { echo 'missing pin: atomic state write'; exit 1; }
grep -qF 'the scheduled automation prompt is ONLY the content' "$S" || { echo 'missing pin: dispatch slice payload'; exit 1; }
grep -qF 'dispatched only when the checkout' "$S" || { echo 'missing pin: D2 default-branch precondition'; exit 1; }
grep -qF 're-evaluate G1 against that final slot' "$S" || { echo 'missing pin: final-slot G1 re-check'; exit 1; }
grep -qF 'fall back to the scheduler prompt span match only when the recorded id is null' "$S" || { echo 'missing pin: id-first parent exclusion'; exit 1; }
grep -qF 'duplicate-parent-candidate' "$S" || { echo 'missing pin: duplicate parent tripwire'; exit 1; }
grep -qF 'ENABLED scheduler template span' "$S" || { echo 'missing pin: enabled duplicate-parent tripwire'; exit 1; }
grep -qF 'consecutive_write_failures' "$S" || { echo 'missing pin: turn tripwire memory-note streak'; exit 1; }
grep -qF 'the two field lines after the block are fill-in spec' "$S" || { echo 'missing pin: authoring dispatch slice'; exit 1; }

# Fixed guard and decision order, by first-occurrence line number:
# G1 (child lane) before G2 (failure cap) before G3 (joint state);
# D1 (execute) before D2 (author) before D3 (no-op).
L_G1=$(grep -m1 -nF 'G1 (child lane)' "$S" | cut -d: -f1)
L_G2=$(grep -m1 -nF 'G2 (failure cap)' "$S" | cut -d: -f1)
L_G3=$(grep -m1 -nF 'G3 (joint state)' "$S" | cut -d: -f1)
test "$L_G1" -lt "$L_G2" || { echo 'order broken: G1 (child lane) not before G2 (failure cap)'; exit 1; }
test "$L_G2" -lt "$L_G3" || { echo 'order broken: G2 (failure cap) not before G3 (joint state)'; exit 1; }
L_D1=$(grep -m1 -nF 'D1 (execute)' "$S" | cut -d: -f1)
L_D2=$(grep -m1 -nF 'D2 (author)' "$S" | cut -d: -f1)
L_D3=$(grep -m1 -nF 'D3 (no-op)' "$S" | cut -d: -f1)
test "$L_D1" -lt "$L_D2" || { echo 'order broken: D1 (execute) not before D2 (author)'; exit 1; }
test "$L_D2" -lt "$L_D3" || { echo 'order broken: D2 (author) not before D3 (no-op)'; exit 1; }

# Consumed primitives exist (the skill consumes them as-is; no new scripts).
test -f scripts/plan_readiness.py && test -f scripts/quota_window_probe.py && test -f scripts/done-lock.sh
...
grep -qF 'one-shot delayMinutes automation' "$Z" || { echo 'missing pin: child shape'; exit 1; }
grep -qF 'recurring: false' "$Z" || { echo 'missing pin: one-shot flag'; exit 1; }
grep -qF 'never push to origin' "$Z" || { echo 'missing pin: parent prompt push rule'; exit 1; }
grep -qF 'treat the lane as busy' "$Z" || { echo 'missing pin: busy-lane fail-safe'; exit 1; }
grep -qF 'per the SKILL.md Step 5 dispatch slice' "$Z" || { echo 'missing pin: zcode dispatch slice pointer'; exit 1; }
grep -qF 'disable or delete the stale parent' "$Z" || { echo 'missing pin: re-arm stale parent removal'; exit 1; }
...
grep -qF 'docs/history/backlog/completed/2026-09-13-maintenance-scheduler-skill.md' "$P" || { echo 'missing pin: blueprint provenance'; exit 1; }
...
grep -qF 'Do NOT start Phase 0' "$P" || { echo 'missing pin: scheduler-role guard'; exit 1; }
grep -qF 'Before Phase 0: verify no merge or rebase is in progress and the done lock is free' "$P" || { echo 'missing pin: execution fire-time checkout gate'; exit 1; }
grep -qF 'run the Step 0.5 readiness gate' "$P" || { echo 'missing pin: pre-step gate'; exit 1; }
grep -qF 'cap 5 total rounds' "$P" || { echo 'missing pin: review loop cap'; exit 1; }
grep -qF 'Never push.' "$P" || { echo 'missing pin: execution push rule'; exit 1; }
grep -qF 'do not create a new branch' "$P" || { echo 'missing pin: authoring branch rule'; exit 1; }
grep -qF 'do not create or schedule further automations' "$P" || { echo 'missing pin: authoring chaining guard'; exit 1; }
...
- [x] Create `agents/skills/maintenance/`; copy `agents/skills/plans/LICENSE.txt` into it byte-identical (MIT; personal email stays only in the LICENSE copyright line).
- [x] Write `SKILL.md` with frontmatter `name: maintenance` and a description covering: schedule and run unattended maintenance turns that process the backlog and plan queue; trigger phrases "maintenance run", "scheduler turn", "process the backlog and plans", "schedule next plan work".
- [x] `SKILL.md` section "Configuration (from facts document)": keys `plans_dir` (default `docs/plans/`), `backlog_dir` (default `docs/history/backlog/`), `facts_path` (default `.ai-playbook/facts.md`), `child_lane_spacing_hours` (default 5), read from the opening TOML block of `.ai-playbook/facts.md`; list only keys the scheduler turn reads.
- [x] `SKILL.md` section "The scheduler turn" with Steps 0 through 6 in this order: Step 0 context load (repository root, facts keys, re-check git state because parallel sessions share the checkout); Step 1 survey (`find docs/history/backlog -maxdepth 1 -name '*.md'` for open items, `find docs/plans -maxdepth 1 -name '*.md'` for open plans excluding the `completed/` and `deferred/` subdirectories, `python3 scripts/plan_readiness.py <plan>` per open plan as the certification oracle phrased as "the exit status of scripts/plan_readiness.py", plan-coverage check by grepping top-level plans for the item's filename, memory index read when the runtime provides one); Step 2 guards in fixed order: `G1 (child lane)` trips when ANY child automation (authoring or execution, classified per the `zcode.md` markers) is armed to fire within `CHILD_LANE_SPACING_HOURS` (default 5, sized to outlast a typical child run; observed runs span one to four hours) of the proposed slot, or fired within that same window per the automation listing's last-run timestamp and may still be running, or a live child session is otherwise discoverable (an execute-plan claim via the runtime API, or active child session traces on the checkout); the markers are a shortcut, not a necessary condition: any enabled automation of this repository, other than the recurring parent automation that fired this turn, whose fire or last-run time falls inside the proposed slot's spacing window is lane-occupying unless its visible prompt is affirmatively unrelated to plan/backlog work, and a fired child fewer than 6 hours past its `fire_at` (computed from the automation listing's fire and last-run fields) also keeps the lane busy; when uncertain whether a child is in flight, treat the lane as busy; `G2 (failure cap)` trips when an alert is present in the state file or recovered from the memory index read-back, from either the child cap (three consecutive no-progress child outcomes) or the turn tripwire (three consecutive `turn_error` records, or an N >= 3 write-failure streak note); `G3 (joint state)` trips when a merge or rebase is in progress on the checkout or the done-lock is held, standing down the whole turn; Step 3 decision: `D1 (execute)` schedules an execution child for the best open plan when G1 passes and an open plan exists (memory dependency-chain order when available, otherwise the oldest basename among digest-intact plans, otherwise the oldest open plan whose re-certification the child's PRE-STEP performs; if the memory index marks every open plan dependency-blocked, fall through to D2); `D2 (author)` schedules an authoring child for the highest-priority plan-uncovered open backlog item when G1 passes and no open plan awaits execution (or every open plan is dependency-blocked per memory), and is dispatched only when the checkout's current branch is the repository default branch (otherwise resolve to D3 with the branch name as the reason); `D3 (no-op)` records the reason and schedules nothing; a tripped G1 always resolves to D3, never to D2, because the authoring blueprint commits on the current branch and would contaminate an in-flight execution child's Phase 0 feature branch on the shared checkout; Step 4 quota leg: run `python3 scripts/quota_window_probe.py`, use its window data to time the child when usable, and apply the fallback when it is not (Task 2 pins the fallback wording); Step 5 scheduling: assemble the child prompt from `prompt-templates.md`, schedule exactly one one-shot child firing at least 30 minutes out via the runtime's scheduling primitive, never more than one child session per scheduler turn and never while any child is in flight (G1); after the quota leg fixes the final fire time, re-evaluate G1 against that final slot before creating the automation (a trip resolves to D3 and schedules nothing); Step 6 state update (schema below, whole-document rewrite from the fresh survey through a temp file plus atomic replace, best-effort: a failed state write is reported in the turn output but never blocks the already-made decision).
- [x] `SKILL.md` section "Failure detection and the failure cap": the child-outcome check runs at the end of Step 1 (survey), so its outputs are on file before Step 2 evaluates the guards in fixed order and `G2 (failure cap)` arms on the tripping turn; each turn checks the oldest `pending` child whose `fire_at` plus 6 hours has passed; progress for an authoring child means a top-level plan now references the target item or the item left the backlog top level; progress for an execution child means the target plan left the top-level plans directory (archived); otherwise, and only when no armed child targets the same work, the child is `failed`; progress resets `consecutive_failures`, failure increments it; at three consecutive no-progress child outcomes the turn writes `alert` (with `tripped_at` and reason) and stops scheduling children until a human clears the alert; the turn also notes the alert in the agent's persistent memory index when one exists. A turn-level tripwire covers the broken-parent case: Step 6 runs in a best-effort guard of its own, so even a turn that fails before scheduling records `last_run_at` and a `turn_error` reason, and three consecutive turns with no successful state update (or three consecutive `turn_error` records) write the same `alert` and stop scheduling children until a human clears it.
- [x] `SKILL.md` section "State file" with the schema below and the scoping note: `.ai-playbook/scheduler-state.json` is advisory for concurrency; the single child lane and joint-state safety come from the runtime's automation listing, the done-lock, and claim checks, never from the state file; `G2 (failure cap)` is the one guard that reads the state file, so a lost or truncated state file resets the cap and re-enables scheduling; the file is single-writer by convention (only scheduler turns write it, peers may read it), every turn rewrites the whole document from a fresh survey through a temp file plus atomic replace, and a lost update degrades only the failure-cap rail; the `children` array keeps the last 20 entries; `parent_automation_id` is filled at initialization with the recurring parent's automation id, and a scheduler turn never treats its own parent as a lane occupant.
...
- [x] Add one row to the first-party skill catalog table, next to the `execute-plan` and `plans` rows: name `maintenance`, path `agents/skills/maintenance/SKILL.md`, one-line summary ("Unattended scheduler turn that surveys backlog, plans, and quota, then schedules at most one child session."), details column naming the decision order (execute the best available plan, digest-intact preferred, then author for the highest-priority uncovered backlog item, then no-op), the single child lane, the failure cap, and the runtime overlay `agents/skills/maintenance/zcode.md`.
- [x] Commit: `docs: catalog maintenance skill in README`

### Task 4: final validation

Files: none (gates only)

- [x] Run the full `## Validation Commands` block from the repository root; expect exit 0 with every pin green.
- [x] Commit: none (validation task; the checkbox marking rides the next commit)
