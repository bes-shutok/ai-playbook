#!/usr/bin/env bash
# Mechanical pins for the maintenance scheduler skill (review r1 fix round).
...
# re-arm paragraph parity, the parent title's single creation source, section
# anchors other files navigate by, SKILL.md runtime-agnosticism, and the
# pricing seed presence. Exit 0 = all pins hold; exit 1 = first failures
# printed. Repo-relative paths only; run from anywhere.
set -u
fail=0
...
if "cron expression" in norm(paras[0]):
