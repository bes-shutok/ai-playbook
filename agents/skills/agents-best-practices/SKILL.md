...
metadata:
  ...
  upstream: "https://github.com/DenisSergeevitch/agents-best-practices"
---
...
### With `how-to-write-skills` skill (consumer)

When the harness includes custom Agent Skills in a managed registry, point authors to `how-to-write-skills` for portable skill structure, `LICENSE.txt`, facts-key externalization, hygiene scanning, and tool-agnostic skill bodies. Read `references/skills-and-connectors.md` in this skill for governance and eval strategy; read `how-to-write-skills` for how this repository authors and vendors skills.

### With `learn` skill (consumer)

When repeated agent failures surface during harness operation, `learn` captures lessons into documentation and enforceable instruction rules. Read `references/agent-legibility-feedback-loops.md` in this skill for the general pattern (mechanical invariants, source-of-truth artifacts, recurring cleanup); invoke `learn` when failures should compound into the project's instruction corpus.

### With `execute-plan` and `plans` skills (related implementations)

This playbook's `plans` and `execute-plan` skills implement a concrete coding-agent workflow orchestration pattern (durable plan artifacts, per-task workers, review loops, budgets). Use `references/workflow-orchestration.md` and `references/planning-and-goals.md` here for harness theory; use those skills for day-to-day implementation in this registry.

### With `handoff` skill (related)

User-requested session handoffs use the `handoff` skill (structured next-session doc under `{tmp_dir}/handoff/` when repo facts exist). Harness auto-compaction uses a similar summary shape in `references/context-memory-compaction.md`; that path is operational inside long runs, not an explicit user handoff.
