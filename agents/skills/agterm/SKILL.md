...
when_to_use: >
  ...
  AGTERM_SESSION_ID, AGTERM_SOCKET, and asks to drive or script agterm. Also: launch an agent CLI
  (Claude Code, Codex, Cursor's agent CLI, Copilot CLI, ZCode, OpenCode, Pi) into an agterm session,
  resume or reattach an agent session across an agterm restart, agent status hook, agent-status
  wiring per runtime, which agent status hooks are installed, AGTERM_AGENT_RE, agent-runtimes. Also: agterm cookbook,
...
allowed-tools: Bash(agtermctl *), Bash(bash *show-image.sh *), Bash(jq *)
---
...
Per-runtime detail (launching, resuming, and the agent-status hooks for Claude Code, Codex,
Cursor's agent CLI, Copilot CLI, ZCode, OpenCode and Pi) is in **agent-runtimes.md**.
...
- `restore ("cmd" | --none | --clear) [--pane left|right] [--pane-id TOKEN]`, pin what a pane re-runs on
  ...
  next launch (it never touches the running session), and STICKY: it fires again on every restart until
...
**surface**, `zoom [show|hide|toggle] [--target surface:<session-id>:left|right|scratch|overlay|overlay-left|overlay-right|quick] [--window W]`, zoom a terminal surface to fill the window (sidebar hidden; a slim title-bar strip with an exit
...
the skill came from: a plugin install or the runtime's skill directory (`~/.claude/skills/agterm/`,
`~/.cursor/skills/agterm/`, `~/.agents/skills/agterm/`, `~/.codex/skills/agterm/`,
`~/.copilot/skills/agterm/`, the app's **Help ▸ Install Agent Skill…** copy; the full list is in
agent-runtimes.md).
...
- **agent-runtimes.md**, per-runtime integration for the agent CLIs that run inside agterm
  ...
  lines, the `session restore` reattach pattern, where each runtime's agent-status hook is wired,
  `AGTERM_AGENT_RE`, and where this skill installs per runtime.
