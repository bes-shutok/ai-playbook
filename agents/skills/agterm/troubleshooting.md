...
- **It already fired once this launch.** The override is consumed once per launch: after it runs, a second
  ...
  is still pinned; `tree` reports `restoreCommand`; and it fires again on the NEXT restart.
...
Install the hooks from Help ▸ Install Agent Status Hooks…. Where each runtime's hook is wired, what
the installer writes per config file, and the Pi/OpenCode load prerequisites are tabulated in
**agent-runtimes.md** (its "Agent-status wiring per runtime" section); diagnose here, in order:

1. **Identify the route.** Shell-integrated agents (the `AGTERM_AGENT_RE` regex route) need a FRESH
   shell so the installer-added `source` line takes effect. `cursor-agent` is in the default regex;
   `copilot` and anything else without a dedicated hook must be added to the regex yourself.
2. **Hook routes** (Claude Code, Codex, ZCode, OpenCode, Pi): check the runtime's own config lists
   the agterm entries (agent-runtimes.md names each file). Absent entries mean the installer was
   skipped or the config was replaced; re-run Help ▸ Install Agent Status Hooks… or add the
   entries by hand. Then restart the runtime so hooks reload.
3. **The wrapper's agtermctl resolution.** The installed wrapper resolves the bundled `agtermctl`
   itself; a bare development build instead needs `agtermctl` on `PATH`. Moving or replacing
   agterm.app invalidates the path the installer baked in; the wrapper then falls back to
   `agtermctl` on `PATH`, and with nothing there the glyph silently stops updating. Re-run Help ▸
   Install Agent Status Hooks… after moving the app, or install the CLI with Help ▸ Install
   Command Line Tool….
4. **Test the wrapper by hand** from a session shell, as agent-runtimes.md shows: if it works by
   hand but the hook does not fire, the problem is the runtime's hook registration, not agterm.
