...
4. **Decide the scope**: after generalizing, apply the FIRST discriminator - **abstract precept vs concrete lesson** - then assess portability. Do **not** treat "not useful in every language" as proof the rule is project-specific. Five-branch fork (resolve `shared_docs_dir` from `user_facts_path`):
   ...
   - **(2b) Company-wide convention -> `company_guidelines_master` (facts key)**: a do/do-not rule or convention shared by repositories of the same company or organization because they perform the same kind of work (same integration responsibility, shared platform or delivery conventions), even when their product domains differ and the rule is not universal across organizations. Place the **full rule** in the company guidelines master and keep at most a concise project witness plus a pointer in the incident repo's project corpus; sync repo mirrors after editing the master (Company rule workflow, Step 2). Add a one-line cross-reference in instruction files per the Step 6 placement ladder. **Sibling search is required before choosing fork (4)**: when the incident repo is company-scoped (under `company_projects_root` per facts; ownership-scoping resolution test in item 5c), grep the company guidelines master for the lesson's topic keywords first; a hit is strong evidence the company scope applies. Under the company root with the master file missing (config drift), do not treat the failed or empty grep as evidence against company scope; route through the item 5c blocking placement question. Outside the company root (a personal repo, or a repo under neither workspace root) the company master is out of scope: skip the sibling search and do not select fork (2b); if the rule still looks company-scope, follow item 5c (blocking placement question) instead of defaulting to fork (4).
...
4c. **Company-portability gate (required before fork (4) full project lesson).** Ask: "Would this rule guide another repository in the same organization that performs the same kind of work, even if its product domain differs?" If **yes** and the master resolves per the item 5c ownership-scoping resolution test, you are in fork **(2b)**: write the full rule to `company_guidelines_master` first, keep only a concise project witness and pointer in the project corpus, and sync repo mirrors. If **yes** but the master does not resolve, stop and report the item 5c blocking placement question. If you almost chose fork (4) because the incident happened in one company repo, re-run this gate after the 4b stack-portability gate. Anti-pattern: storing a company-wide integration convention as a full project lesson because "it is not universal across every possible project" (that is fork (2b), not fork (4)). A team or infrastructure ownership boundary (which team owns which tickets, components, or delivery stages) binds sibling repos with the same responsibility, so it is company scope, not project scope. If no, continue down the ladder: fork (3) when the witness is the reusable value, fork (4) only with a stated residual dependency.
5. **Place accordingly**: write the full text at the chosen scope. If the rule goes to shared canonical docs (fork 1, 2, or 2b), reduce the project instruction / project corpus entry to a cross-reference (plus optional short witness). If it stays project-level (fork 4), write the full lesson in the project corpus.
5b. **Mirror-fan-out when this skill's fork changes:** if you renumber or retarget the placement fork (item 4 / 4b), grep sibling skills for `mirrors learn`, `three-way fork`, and `fork 2 vs fork 3`, and update every living claim that mirrors this pass (especially `generalize`) in the **same change set**. The same fan-out applies when the ownership-scoping resolution test changes (living surfaces: facts `Guideline canonical homes`, item 2b, item 5c, the Completion Checklist sibling-search line, `done` Step 3 item 4a). Also update living UL See also lines that cite item 4 by those old labels. Do not leave a skill claiming it mirrors learn while still describing the previous fork.
5c. **Placement receipt (required in the learn run output).** For every placed lesson, state the selected scope, why each alternative scope was rejected, and the ownership-scoping resolution outcome (master resolved, outside the company root, or config drift; one line per lesson; for example "fork (2b): company-wide convention; master resolved; not universal (org-specific), not fork (4) (guides sibling repos with the same responsibility)"). Evaluate the residual dependency and the canonical-destination resolution BEFORE executing item 5's corpus write: item 5c conditions the write, it does not follow it. For a fork (4) placement the receipt must also state the residual dependency that keeps the rule project-specific (a material dependency on the service domain, module contract, repository architecture, or project-only operational behavior). If no residual dependency remains, do not write the full lesson to the project corpus: re-route to fork (2b) or fork (3). When the rule is company-scope but `company_guidelines_master` does not resolve, report the missing canonical destination as a blocking placement question instead of writing a full project lesson. The question and the routing answer appear in the run output as a one-line receipt (for example `company-scope rule; master unresolved (config drift): routed to <scope> per user`), and the receipt states the reason: outside the company root, or config drift when the repo is under the root but the key's path is missing. The routing answer respects item 2b: outside the company root, fork (2b) is not offered even on request (deferral to a company repo is the preferred routing; the user may instead select fork (3) or (4)). **Ownership-scoping resolution test:** the master resolves only when the incident repo (per item 3b) sits under the company workspace root (`company_projects_root` in facts) AND the key's path exists. Containment compares fully resolved absolute paths component by component: resolve symlinks, expand the tilde, strip trailing separators, normalize Unicode to a single form, and normalize case when the filesystem is case-insensitive (mechanics mirrored in the facts `Guideline canonical homes` paragraph; update both per item 5b). A repo at the root itself is not under it, and a directory sharing only a leading substring with the root does not match. Outside the company root (a repo under the personal root, or under neither workspace root) the key is treated as unresolved even though the user-level facts table lists a global value; under the root with the path missing it is also unresolved (reason: config drift), which `done` Step 3 item 4a audits at commit time as a non-blocking drift WARNING. The receipt is temporary run output; it need not become part of the canonical lesson, but `done`'s pre-commit scope audit uses it as placement evidence.
...
- company-portability gate (Step 1.2 item 4c) ran before every project-specific placement, and the sibling search of `company_guidelines_master` ran before every full project lesson wherever required by items 2b/4c/5c (not run outside the company root; under the root with config drift the grep may return empty without counting against company scope, per item 2b); a placement receipt (item 5c), stating the residual dependency for every fork (4) placement, was emitted for every placed lesson
