...
- On `pause`, run the pause protocol:
  ...
  2. Keep the written guard flag armed. The guard flag is host-global: while armed it gates every session's tool calls on the host until expiry or manual removal, mirroring the hook README's host-scope note. Exception: when the binding limit is the weekly `secondary` window, the probe writes no flag at all (nothing is armed); the record applies to every binding window, so continue with step 3 and skip only the scheduling steps (4-6).
  ...
  7. When the binding limit is the weekly `secondary` window, do not schedule (step 3's record was already written): report for user decision.
...
4. Asks the runtime driver to atomically claim the task, then launches the worker through the selected adapter (never parallel for implement/done/review-fix).
...
**Terminal receipt (before removal):** The runtime driver's terminal operation is the gate, never the Markdown audit receipt. While `{tmp_dir}/execute-plan/<PLAN_SLUG>/manifest.md` still exists, call the continuation driver's terminal operation with the archived-plan path, the last commit SHA, and the Phase 5 success checklist; then re-read the receipt the driver returns and confirm it. Only after the driver-verified receipt exists, mirror the verified fields (`workflow_state: complete`, archived-plan path, last commit SHA) into `manifest.md` as the human audit receipt; that synchronization step is never the gate. Capture the verified receipt fields into the final user report. Do **not** delete the session directory before the driver terminal receipt is written, re-read, and mirrored. A missing session directory or an `active` machine manifest is evidence that execution is still in progress, not a reason to return a final answer.
