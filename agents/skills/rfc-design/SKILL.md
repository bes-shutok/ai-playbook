...
13. **Encrypted edge-auth directions**: when an API gateway or platform authorization hop carries encrypted traffic, a §3 or §4 diagram shows expected ciphertext vs cleartext directions; prose-only hop lists are not enough. Mermaid `sequenceDiagram` messages must not contain `;`.
