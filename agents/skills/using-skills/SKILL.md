...
6. **Plan file reference** - A path under the project `{plans_dir}` alone is not an execute-plan trigger. Resolve doc paths per `_shared/doc-paths.md`. Route through the plan-path gate in the `execute-plan` skill (execute-plan / manual / read-only) before editing production code.
7. **Documentation paths** - Skills that read/write repo docs resolve `{plans_dir}`, `{reviews_dir}`, `{tmp_dir}`, etc. from project specs (`project_guidelines_rel`, `AGENTS.md`) — see `_shared/doc-paths.md`. **`doc-hierarchy-migrate`** applies the three-layer schema when explicitly migrating a repo; **`doc-hierarchy-upkeep`** updates Layer 1/2 after migration; **`doc-hierarchy`** is schema reference only.
