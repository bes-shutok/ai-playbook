...
9. **Design RFCs** - When the user asks to create, draft, edit, or review a Design RFC or feature design in Markdown, invoke **`rfc-design`**. Confluence-hosted RFC/TDD pages use **`review-confluence-doc`**.
10. **Review loop** - When the user asks to repeat review until clean on the current branch (not `execute-plan` Phase 3), invoke **`review-loop`**. Exit only when a **fresh** `doing-code-review` reports zero Medium+ **before** fixes; post-fix cleanliness does not count.
