...
- **Classification tag rule:** every task checklist item carries a classification tag at the end of the item line: `[class: IMPLEMENTATION_REQUIRED]` for implementation work, `[class: REPOSITORY_TEST]` for in-repo verification work. External-gate and operations requirements are never task items; they are written in **Ship when** prose with their class (`[class: EXTERNAL_RELEASE_GATE]` or `[class: OPERATIONS_FOLLOW_UP]`), their evidence owner, and their closure condition named in the prose. `plan_readiness.py` enforces the tag rule under the sidecar-date rule (see `scope_classification_problem` in scripts/plan_readiness.py for the vocabulary, the Ship-when exemption, and the exact date).
- The taxonomy mapping and conflict rule: the four classification tags refine the inclusion gate's classes. `IMPLEMENTATION_REQUIRED` and `REPOSITORY_TEST` refine the repository-implementation class; `EXTERNAL_RELEASE_GATE` refines the release-condition class and `OPERATIONS_FOLLOW_UP` refines the external-prerequisite class. On conflict the stricter inclusion-gate classification wins: an item carrying a gate or operations obligation is classified by the stricter class and moves under **Ship when**.
- **Scope ledger (prevention):** classification is recorded at plan creation and re-checked at the first readiness review, so misclassification surfaces at the readiness boundary, not mid-execution.
- **Ownership ledger (prevention):** the ownership ledger of files, witnesses, and their owning tasks is written before prose expansion, so method-level ownership ordering is settled at authoring time rather than rediscovered mid-execution.
...
18. **Probe pattern distinctiveness and scope congruence:** A dedicated grep is dedicated only if deleting the guarded sentence breaks it. Quote a distinctive multi-word span verbatim from the normative rule line and verify the span is unique in the file; a single common token that also appears in unrelated prose of the same file aliases to that other text and stays green when the obligation is deleted. Match the probe's region to the obligation's region: when an obligation is scoped to a sub-region (frontmatter, one section), sweep exactly that region rather than the whole file (the term may legitimately appear elsewhere), extract the region with a fail-closed assertion on an anchor line (for example `^name:`) so a broken extractor cannot pass vacuously, and make forbidden-term sweeps of the region case-insensitive (see `development_lessons.md` #187). The converse also holds: when the obligation is whole-file presence of a string (a sentinel message, a guard phrase), do NOT couple the probe to a span window around an anchor (`grep -A N` after a function definition): a behavior-preserving refactor legitimately relocates the string into a helper and breaks the span while the obligation still holds; probe presence file-wide and, when the delegation itself matters, add a separate probe for the call site (witness: a read-only drift probe assumed a conflict sentinel within 10 lines of its entry-point function and false-flagged drift after the sentinel was extracted into a shared failure helper).
19. **Wrap-tolerant forbidden-phrase sweeps, proven RED today:** a multi-word prose phrase can be line-wrapped in the target document, so a line-based grep can NEVER match it and a "no remaining hits" sweep false-passes forever. Flatten before matching (`tr '\n' ' ' < file | tr -s ' ' | grep -q "<phrase>"`) or sweep a single-line fragment; the `tr -s ' '` squeeze is load-bearing when continuation lines carry leading indentation (wrapped docstrings/comments), which leaves multi-space runs in the flattened text that a single-space fixed-string cannot match; when the target text lives inside a comment block, strip the comment marker BEFORE flattening (`sed -e 's/^[[:space:]]*#[[:space:]]\{0,1\}//'` first): the `#` survives newline-flattening and splits the phrase, and the same treatment applies to positive-presence pins over wrapped comments, not only forbidden sweeps (a scratch-tree simulation of a task's prescribed comment text caught the plain-flatten pin failing on the correctly implemented tree); and at authoring time EXECUTE the sweep against the CURRENT content and record that it fires (the gate must be RED-today, flipping GREEN exactly when the rewrite task lands). A reviewer's "verified empirically" claim is not proof; the next round re-executes it (see user-level lessons #220: a vacuous sweep survived three review rounds plus a false empirical-verification claim; project witness: homelab-llm lesson 36, where newline-only flattening still missed indented docstring wraps). A gate whose target files do not exist yet passes vacuously on the missing paths: when a plan creates the files it gates, execute the WHOLE validation block and record the FIRST actually-failing gate (with its exit code) plus each vacuous pass, never a predicted gate number (see user-level lesson #337).
...
Plan authoring runs the same long review loops that quota exhaustion kills mid-flight, so this skill carries its own quota-window pause-and-resume gate. Resolve `budget_pause_minutes_before_reset` (fallback `20`), `budget_pause_max_used_percent` (fallback `90`), `budget_pause_min_protocol_minutes` (fallback `10`), and `budget_probe_runtime` (fallback `auto`, unchanged semantics) from the opening TOML block of `.ai-playbook/facts.md`.

**Boundaries:** the orchestrator runs the probe before launching each review-plan round in the Plan Quality Gate loop, before every fold batch and every mechanical audit pass inside the loop (the between-boundary probes), and before the final done handoff; never mid-worker and never mid-fold. The runtime budget-guard backstop hook (`agents/hooks/budget-guard/`) enforces the pause at the tool-call level once registered (fixture-verified on one runtime; pending the first-start trust-prompt approval and the deny-envelope drive on the other runtime, see the budget-guard README), so a mid-round quota exhaustion cannot silently burn the window.

unsupported harness budget skip: before resolving the probe, run scripts/harness_detection.py (resolved like BUDGET_PROBE minus its env-override leg: repo-local copy first, then the deployed home copy); when it reports `harness: none` and no `budget_probe_runtime` override is configured, record `budget_skip` in `{tmp_dir}/plan-requirements-<slug>.md`, skip the probe, the pause protocol, and the resume watcher, and continue, leaving any existing pending resume watcher to its own stand-down checks; the skip path never arms or clears the guard flag (when a `budget_probe_runtime` override routes the run back through the probe, the probe's own flag semantics apply, and that override is the only way to run the family on an unsupported harness); a missing or erroring helper counts as unsupported (record the evidence; the family must not run against an unknown harness).

At each boundary, resolve and run the probe:

```bash
# Three-step probe resolution mirroring the execute-plan Budget gate: BUDGET_PROBE env override first; then the repo-local `scripts/quota_window_probe.py` when present; then the deployed $HOME/.ai-playbook/scripts/quota_window_probe.py.
...
- When the notes name a comparable earlier round panel's observed window cost, pass it as --plan-cost <percent> and follow the report's wave_recommendation (full / split with wave_size / pause; on pause run the pause protocol instead of launching the round). A pause decision always outranks any wave_recommendation: on pause_decision: pause run the pause protocol instead of launching. Without a recorded comparable cost, omit the flag.
- On `pause_decision: continue`, continue the normal flow. A missing or unopenable probe script is treated as `status: unknown`, noted in `{tmp_dir}/plan-requirements-<slug>.md`, and the flow continues; it must never block authoring.
- The `--write-flag` argument makes `write_flag_if_paused` arm the guard flag (`~/.ai-playbook/runtime/budget-guard.flag`) on a pause decision, so the registered backstop hook has something to enforce. The weekly `secondary` binding still writes no flag and schedules nothing.
- On `pause_decision: continue` with a trusted binding reset epoch, schedule the same standing resume watcher the canonical Standing resume watcher subsection defines, at the authoring boundaries (before launching each review-plan round in the Plan Quality Gate loop; at every between-boundary probe inside the loop; before the final done handoff): reset time plus one minute, self-disarming, replaced at each later boundary, stood down on progressed, peer-resumed, archived or completed, or aborted or interrupted where the authoring runtime records an interrupt (same newer-than-scheduling comparison against the authoring machine state, so a latched old interrupt never trips a later watcher); otherwise the scheduling-snapshot and plan-state checks carry the stand-down. This is the known-binding-only watcher branch: a `status: unknown` report or a weekly `secondary` binding schedules no watcher, and every non-schedulable boundary (unknown, weekly secondary, pause, abort, complete) supersedes and clears a pending watcher per the canonical record rule. The authoring pause-protocol fallback chain is unchanged (the sentinel self-disable launchd case and the report-only naming stay as written above).

**Authoring watcher state:** the machine-state home for that mirror is `{tmp_dir}/plan-requirements-<slug>.json`: it stores `progress_revision`, `user_interrupt`, and the same fenced `resume_watcher` receipt as the canonical protocol (watcher id, plan slug, canonical repository root, canonical plan path, plan-byte digest, scheduling epoch, reset epoch, expected generation, expected progress revision, current boundary generation, binding, status, and replacement predecessor), written under the shared watcher lock with the same compare-and-swap replacement and cancellation fences. `{tmp_dir}/plan-requirements-<slug>.md` remains the human notes and audit projection: it receives a projection line only after a lock-held state transition, and the watcher never infers authoring progress from Markdown `updated:` text. The authoring loop updates the machine state under the shared watcher lock before and after each review round and done handoff, so a scheduled watcher's expected progress revision goes stale exactly when authoring moved. The plans `WatcherStateAdapter` in `scripts/execute_plan_resume_watcher.py` invokes the one shared watcher state machine and shares the same replacement, cancellation, boundary generation, canonical plan path and digest, semantic-progress, peer-resume, archive, completion, abort, and interrupt fences as the execute-plan runtime adapter (the plan digest is recomputed from the canonical plan-file bytes at every watch); no second watcher state machine exists in prose. The machine state is driven through the driver CLI's manifest-free authoring operations (r2 F11), each taking the payload `state_path` naming `{tmp_dir}/plan-requirements-<slug>.json`: `plans-watcher-schedule` records the boundary decision and runs the fallback chain at this boundary, `plans-watcher-supersede` clears a pending watcher, `plans-watcher-fire` is the automation prompt's fire entry over the authoring state, `plans-resume-marker` records a resume-path re-entry so a scheduled watcher stands down on the peer fence, and `plans-progress`, `plans-interrupt`, and `plans-terminal` record the progress revision, the user-interrupt fence, and completion, archival, or abort (the operation shapes are pinned in the execute-plan runtime contract's CLI boundary section).
- On `pause`, run the pause protocol:
  1. Finish the current boundary only; do not start new work.
  2. Keep the guard flag armed (it is host-global: while armed it gates every session's tool calls on the host until expiry or manual removal). When the binding window is the weekly `secondary`, write no flag, still append the record (step 4) with resume_scheduled: no, schedule nothing, and report for user decision instead.
  3. On a host whose agent runtime supports one-shot scheduled automations, create the resume automation FIRST at the reset time rounded up to the whole minute plus one, then record the boundary through `plans-watcher-schedule`: at this boundary the pause decision supersedes and clears any pending watcher, writes no `resume_watcher` receipt, and runs no scheduler chain (the canonical record rule; a pause boundary is non-schedulable), so the created automation - not the driver - carries the resume. The automation's self-contained prompt carries the same two-half resume contract as the canonical pause protocol's step 5: on a pause boundary (no receipt was written) there is nothing for the fire operation to evaluate, so the prompt performs the epoch-checked manual clear itself - re-enter the plans review loop on the plan path, read this run's `budget_pause` record from `{tmp_dir}/plan-requirements-<slug>.md`, verify the latest review sidecar digest still matches the plan bytes (a budget pause never edits the plan, so no re-certification is needed), clear `~/.ai-playbook/runtime/budget-guard.flag` and `budget-guard.fired` before relaunching only if the flag's `reset_at_epoch` matches the reset epoch recorded in this run's `budget_pause` record (otherwise leave both in place and report the mismatch: the flag encodes a different, still-live window, another runtime's or a newer window of the same runtime; if the flag (or fired marker) is already absent, nothing is armed, so proceed with the relaunch and note the pre-cleanup in the record), re-reading the flag immediately before each deletion and aborting the clear if its `reset_at_epoch` changed since the match check (mirror the hook core's re-read-before-unlink pattern: a concurrent probe may have replaced the flag in between), and stand down if the plan is archived or completed or a peer session resumed the work; this manual procedure is the no-CLI fallback path of the protocol, and on a pause boundary it is the only path. When a pending receipt exists (a watcher scheduled at a continue boundary), the prompt's fire contract is the authoring fire entry `plans-watcher-fire` (the manifest-free CLI over the payload `state_path`): the operation applies the fences and the four stand-down checks and clears the guard flag and fired marker only on a resume decision, so the prompt must not re-apply the stand-down checks or delete the guards by hand. A cleanup call issued before the window resets may itself be denied once by the guard; retry it after that single block.
  4. Append a `budget_pause` record (runtime, reset epoch and ISO time, thresholds used (including the protocol margin), next step that would have run, and resume_scheduled: yes|no - `yes` when step 3's orchestrator-created automation exists (the call's supersede leaves nothing else pending); `no` when nothing could be scheduled (the report-only and weekly-secondary cases)) to `{tmp_dir}/plan-requirements-<slug>.md`.
  5. Fall back to a launchd one-shot with a sentinel self-disable file when automations are unavailable on a schedulable boundary (a continue with a trusted binding reset epoch); if neither capability exists, end report-only naming the exact resume command and time. On a pause boundary there is no launchd carrier either (a pause runs no scheduler chain): report the exact manual resume command and reset time and treat the resume as manual, mirroring the canonical pause protocol's launchd-only narration. (This compresses the canonical two-case launchd split into one step; the compression is a declared delta, see the Canonical home note below. The launchd job and the report-only naming are rendered by the shared schedulers in `scripts/execute_plan_resume_watcher.py`, whose arm now bootstraps the job into launchd and treats a failed bootstrap as not-scheduled falling through to report-only, r2 F4; the runtime boundary's exact driver commands stay canonical in the execute-plan pause protocol: `--operation watcher-schedule` records the boundary and `--operation watcher-fire` is the automation prompt's fire entry over `runtime_state.json`, while this authoring boundary drives the same state machine through the `PlansAuthoringWatcherAdapter` over `{tmp_dir}/plan-requirements-<slug>.json` via the manifest-free `plans-watcher-*` and `plans-resume-marker` operations, with `plans-watcher-fire` this mirror's fire entry.)

Every resume path (the scheduled automation's prompt, a fired standing resume watcher, and any manual resume) first reconciles a budget_pause record carrying resume_scheduled: no by completing the scheduling step before continuing work. Exception: a record whose binding is the weekly secondary window stays report-only - report for user decision and never schedule.

**Canonical home:** this section mirrors the execute-plan Budget gate section, which is the canonical home of the shared pause protocol (thresholds, weekly secondary rule, resume timing, flag and fired-marker clearing, fallbacks); on conflict that section wins, and the deltas owned here are the two authoring boundaries, the `budget_pause` record sink, the completed-plan stand-down trigger (a fallback-pair prompt delta only, r1 O24: this mirror's pause-protocol resume prompt stands down when the plan is archived or completed, where the canonical prompt names archived; the shared watcher state machine's archived-or-completed fence is the same comparison in both workflows and is not a delta, with one declared authoring-side input delta, r2 O23: the authoring adapter additionally reads the authoring machine state's `plan_status` field, so `plan_status: archived` alone trips the same stand-down where the runtime manifest has no such field), the mandatory `--plan <plan-slug>` argument at this boundary (plan authoring always knows its plan slug; the canonical invocation keeps `--plan` optional), the fallback compression (the sentinel self-disable file is attached to both launchd fallback cases in this mirror, and the host-clock wording is dropped), and the authoring-boundary watcher deltas (the `{tmp_dir}/plan-requirements-<slug>.json` machine-state home and its Markdown projection-only notes rule defined above).

## Plan Quality Gate

**Budget gate:** run the Budget gate here per the Budget gate section before launching each review-plan round; on a pause the run stops at this boundary; on a continue with a trusted binding reset epoch, schedule the same standing resume watcher per the Budget gate section (known-binding-only branch).
...
11. **Probe each new fold rule in both directions:** when a fold adds or retightens a cross-field rule (verdict gates, link rules, reconciliation rules, required-field rules), the next re-probe must exercise the new rule for BOTH failure directions: a bypass fixture (false yes) AND an over-blocking witness against every ordinary valid flow the rule touches (false no: declared-out-of-scope rows, legacy grandfathering, separate-round flows, partially-filled records), plus the schema vocabulary to represent every state the rule distinguishes. A new rule's missing representation for a legitimate flow surfaces as the next round's blocking finding even when the fold fixed the prior finding correctly (witness: one plan's fold-added reconciliation rule failed focused rounds carrying declared-out-of-scope rows; its link rule was pinned with the inverted direction for the inherited-link kind; its fixture arms asserted gates no task pinned into the validator's duty list; its attempt-outcome enum could not represent admission-denied attempts).
...
- **Claim-to-task ownership sweep (the reverse direction):** every behavior or design claim in `## Gist & Examples`, `## Design Invariants (CR Guard)`, `## Evaluation Criteria`, and the Assumptions decision receipts traces to an owning task checkbox; before the first review round, sweep those four claim sections against the task list. The review-side closure matrix runs task-to-artifact; a claim with no owning task survives every per-task gate silently and surfaces only as a mid-loop blocking finding (witness: an authoring run committed to a fence hoist in four claim sections while no task implemented it; found only at r1).
...
**Budget gate:** run the Budget gate here per the Budget gate section before the final `done` handoff; on a pause the run stops at this boundary; on a continue with a trusted binding reset epoch, schedule the same standing resume watcher per the Budget gate section (known-binding-only branch).
