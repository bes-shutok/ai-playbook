...
- On `pause`, run the pause protocol:
  ...
  2. Keep the guard flag armed (it is host-global: while armed it gates every session's tool calls on the host until expiry or manual removal). When the binding window is the weekly `secondary`, write no flag, schedule nothing, and report for user decision instead.
