...
Plan authoring runs the same long review loops that quota exhaustion kills mid-flight, so this skill carries its own quota-window pause-and-resume gate. Resolve `budget_pause_minutes_before_reset` (fallback `20`), `budget_pause_max_used_percent` (fallback `90`), and `budget_probe_runtime` (fallback `auto`, meaning omit `--runtime` entirely so the probe auto-detects) from the opening TOML block of `.ai-playbook/facts.md`.

**Boundaries:** the orchestrator runs the probe before launching each review-plan round in the Plan Quality Gate loop, and before the final `done` handoff; never mid-round. The runtime budget-guard backstop hook (`agents/hooks/budget-guard/`) enforces the pause at the tool-call level once registered (fixture-verified on zcode; on codex pending the first-start trust-prompt approval and the envelope drive, see the budget-guard README), so a mid-round quota exhaustion cannot silently burn the window.
...
- On `pause`, run the pause protocol:
  ...
  2. Keep the guard flag armed (it is host-global: while armed it gates every session's tool calls on the host until expiry or manual removal). When the binding window is the weekly `secondary`, write no flag, still append the record (the record applies to every binding window), schedule nothing, and report for user decision instead.
  3. Append a `budget_pause` record (runtime, reset epoch and ISO time, thresholds used, next step that would have run) to `{tmp_dir}/plan-requirements-<slug>.md`.
  4. Schedule the resume automation at the reset time rounded up to the whole minute plus one, with a self-contained prompt that re-enters the plans review loop on the plan path, reads this run's `budget_pause` record from `{tmp_dir}/plan-requirements-<slug>.md`, verifies the latest review sidecar digest still matches the plan bytes (a budget pause never edits the plan, so no re-certification is needed), clears `~/.ai-playbook/runtime/budget-guard.flag` and `budget-guard.fired` before relaunching only if the flag's `reset_at_epoch` matches the reset epoch recorded in this run's `budget_pause` record (otherwise leave both in place and report the mismatch: the flag encodes a different, still-live window, another runtime's or a newer window of the same runtime; if the flag (or fired marker) is already absent, nothing is armed, so proceed with the relaunch and note the pre-cleanup in the record), re-reading the flag immediately before each deletion and aborting the clear if its `reset_at_epoch` changed since the match check (mirror the hook core's re-read-before-unlink pattern: a concurrent probe may have replaced the flag in between), and stands down if the plan is archived or completed or a peer session resumed the work. A cleanup call issued before the window resets may itself be denied once by the guard; retry it after that single block.
  5. Fall back to a launchd one-shot with a sentinel self-disable file when automations are unavailable; if neither capability exists, end report-only naming the exact resume command and time. (This compresses the canonical two-case launchd split into one step; the compression is a declared delta, see the Canonical home note below.)

**Canonical home:** this section mirrors the execute-plan Budget gate section, which is the canonical home of the shared pause protocol (thresholds, weekly secondary rule, resume timing, flag and fired-marker clearing, fallbacks); on conflict that section wins, and the deltas owned here are the two authoring boundaries, the `budget_pause` record sink, the completed-plan stand-down trigger (this mirror also stands down when the plan is completed, not only archived), the mandatory `--plan <plan-slug>` argument at this boundary (plan authoring always knows its plan slug; the canonical invocation keeps `--plan` optional), and the fallback compression (the sentinel self-disable file is attached to both launchd fallback cases in this mirror, and the host-clock wording is dropped).

## Plan Quality Gate

**Budget gate:** run the Budget gate here per the Budget gate section before launching each review-plan round; on a pause the run stops at this boundary.
...
**Budget gate:** run the Budget gate here per the Budget gate section before the final `done` handoff; on a pause the run stops at this boundary.
