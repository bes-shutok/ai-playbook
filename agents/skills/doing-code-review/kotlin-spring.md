...
- **Endpoint-owned error taxonomy:** a typed 4xx is not enough when the exception maps to the **wrong module error code** for that route. Match structural request-shape failures to the endpoint's owning module (example: null elements or missing discriminators on a profile/user-updates path use profile-owned invalid-property exceptions, not a sibling module's consent-update exception). Field validation that is clearly sibling-module-op-specific may still use that module's exceptions. Flag `architecture#exception-ownership` or `quality#wrong-endpoint-error-code` when HTTP status is 400 but the wire `code` / handler ownership is wrong for the path.
