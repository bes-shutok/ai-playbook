...
**Writing:** Follow `agent_workflow_guidelines.md` §39 (no em dashes) and the canonical writing contract (§45, 45.10); this skill keeps only its Slack-specific rules. Scan every draft for the U+2014 em dash character before preview or draft-save.
...
- **Destination discovery.** When the user has not fixed the destination, search for the most relevant dedicated channel and active thread before proposing a DM. For a cross-team topic with multiple owners, prefer one relevant thread with clearly separated owner sections over parallel DMs or unrelated threads.
