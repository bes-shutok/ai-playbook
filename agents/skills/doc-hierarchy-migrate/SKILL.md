...
description: >-
  ...
  hierarchy", "(legacy project-specific trigger)". Schema reference:
...
3. Run gates with `REPO_ROOT` set to the **service repo root**; invoke the script from the skill install path:
   ...
   `{skill_install}` = directory containing this skill's `SKILL.md` (e.g. ai-playbook `agents/skills/doc-hierarchy-migrate`). Do not set `REPO_ROOT` to the skill install directory. Paste output before claiming done.
...
| `_shared/doc-paths.md` | Default path map applies only after migration-complete signal |
...
- [`../_shared/doc-paths.md`](../_shared/doc-paths.md), `plans`, `execute-plan`, `learn`, `docs-branch`, `done`
