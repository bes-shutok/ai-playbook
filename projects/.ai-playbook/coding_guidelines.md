...
## 32. Split a Guard's Unsuitability Causes: Out-of-Scope Skips with a Witness, Config Drift Warns Loudly (B)

**Principle:** Family B (error-policy propagation: each unsuitability cause of a guard gets its own outcome direction, matched to its cost of silent failure).

**Shape trigger:** A guard's applicability test (scope check, ownership key, license or feature precondition) can fail for more than one reason, and the guard protects a fail-closed value. Ask: which failure causes are legitimate "not applicable here", and which are evidence the wiring broke?
