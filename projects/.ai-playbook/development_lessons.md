...
## 165. Code Review Company-Rule Findings Use "As Per Company Guidelines" With the Public Playbook URL, Not "Contract or Docs"
...
## 188. A Filter That Re-derives One Field of a Cross-Field-Invariant Dataclass Must Re-derive ALL Coupled Fields in One Replace
...
**Example (2026-07-15 tax-reporting review-flag-aggregation-boundary plan, Task 1):** `CryptoCapitalGainEntry.__post_init__` raises `ValueError("review_reason must be set when review_required=True")`. `_aggregate_capital_entries` previously set `review_required = any(lot.review_required...)` and `review_reason = "; ".join(...)` independently. Task 1 added a post-aggregation filter `_re_evaluate_aggregated_review(entry) -> tuple[bool, str | None]` that drops a stale "zero-basis" review flag when the aggregated cost/proceeds/gain are material. The first sketch returned only the new `review_required` and left `review_reason` carrying the dropped zero-basis text; constructing the result raised `ValueError` because `review_required=False` paired with a non-None reason is fine, but the inverse path (`review_required=True, review_reason=None`) - reached when every survivor is stripped - trips the invariant. Fix: the helper returns BOTH fields and the wiring applies them in one `replace(review_required=..., review_reason=...)`. See plan Invariant 2 ("the helper is the single source of truth for both fields; partial application triggers `ValueError` from `__post_init__`").

**See also:** glossary `docs/maintenance/glossary.md` ("Aggregated review flag", "Zero-basis review"), `src/tax_reporting/application/crypto/entities.py` (`CryptoCapitalGainEntry.__post_init__`), user-level #55 (independent validation fields must NOT be coupled into entry-level `__post_init__` - the inverse of this lesson: this one is about fields that ARE coupled), #56 (per-field aggregation strategy).

## 189. Skills Whose Canonical Script Uses `${arr[@]+"${arr[@]}"}` Must Be Invoked Under bash, Not zsh: In zsh That Guard Expands to a Single Empty Argument and Runs the Loop Body Once With an Empty Variable

**Principle:** Family H (Verify the real thing, not the abstraction: the `${arr[@]+"${arr[@]}"}` idiom is the documented bash-3.2-safe empty-array guard from #166, and the abstraction "this is the portable empty-array idiom" hides that its portability is across bash versions, NOT across shells; in zsh the same syntax expands differently and runs the loop body once with an empty variable). Cross with the skill-portability family: skills authored and tested under bash must declare and enforce their target shell when they reach for shell-specific expansion semantics.
...
**See also:** user-level #166 (the bash 3.2 `${arr[@]+"${arr[@]}"}` idiom under `set -u` - the SAME syntax, the OPPOSITE shell family: #166 fixes a bash-3.2 hazard; this lesson is the zsh hazard that the same idiom does NOT defend against), coding_guidelines.md #25 (Family H parent: verify the real deployment shell, not the "portable idiom" abstraction), `docs-branch` SKILL.md Step 2, `done` SKILL.md Step 2 invocation guidance, CLAUDE.md/AGENTS.md "agent default shell is zsh on macOS; run bash-targeted skill scripts via `bash <file>`".

## 190. A Review Wrapper That Invokes `doing-code-review` as a Single Sub-Agent Cannot Fan Out the Panel; It Silently Collapses to Solo and the "Solo" Statistic Label Masks the Bypass
...
**Rule:**
1. A plan task or Design Invariant that asserts "the code currently does X" or "this placement achieves Y" MUST be verified against the actual source artifact in the same authoring session: read the cited lines, run the production helper if the claim is about runtime behavior (e.g. "is this token in the set", "does this helper raise on None"), or trace the actual branch the input takes. Cite the verification in the plan ("verified at `crypto_reporting.py:764-767`: the else-branch fires").
...
**Shape trigger (when to suspect this family):** A plan-review round returns a Critical finding with an empirically-verified comment (production-helper execution, line-number trace, N-trial simulation) that contradicts a plan claim. The plan's Gist, before/after table, or a Design Invariant asserts current behavior using inherited language ("reach `_parse_capital_gains_file` and land in `context.review_entries`", "this placement makes the lookup-avoidance real", "this is a classifier-regression detector"). A RED test in the plan asserts behavior that already holds today (a "fake test" that cannot go RED).
...
**Example (2026-07-18 tax-reporting `2026-07-18-crypto-dust-partition-fee-skip` plan, three same-session instances):**
1. **Feature-notes premise was factually wrong.** The source `docs/history/feature-notes/2026-07-15-review-flag-deferred-findings.md` item #1 asserted that 99 `FEE` all-zero rows "reach `_parse_capital_gains_file` and are appended to `context.review_entries` via the all-zero + known-token branch at `crypto_reporting.py:737-763`, producing 99 WARNING log lines, 99 review-list entries." The plan inherited this verbatim into its Gist. Plan-review r1 verified empirically: `FEE` is NOT in `popular_crypto_tokens.json`, `_contains_popular_token("FEE")` is False, and `_collect_known_asset_tickers` never adds FEE (all rows are all-zero). So FEE all-zero rows take the ELSE-branch at `:764-767` -> `_register_skipped_zero_asset` -> `skipped_zero_value_tokens`. They do NOT reach `review_entries` and emit ZERO WARNINGs. Two Task 1 RED tests asserted the non-existent pollution behavior (could not go RED), and the entire Part 1 motivation was wrong.
2. **Invariant was tautological.** The plan's Design Invariant 5 framed `len(real_rows) + len(dust_rows) != len(taxable_now_entries)` -> `FileProcessingError` as a "classifier-regression detector." Plan-review r3 ran a 100,000-trial simulation: the invariant fired 0 times across all randomized inputs. The math: the helper computes `real = [not P(e)]` and `dust = [P(e)]` as complementary filters over the same list, so `|[P]| + |[¬P]| == |L|` holds for any deterministic predicate by construction. The guard cannot detect a discriminator change; its test could only be constructed via monkeypatching/non-determinism. The plan's Task 5 test description even admitted this ("monkeypatching the partition or constructing a report whose entries mutate during iteration") - the tell was in the plan itself, unverified.
3. **Placement did not achieve the claimed avoidance.** The plan's Design Invariant 4 claimed the FEE short-circuit was placed "before the popular-token / non-latin lookups at `:729-730` (so the lookup-avoidance is real)." Plan-review r3 verified the line ordering: `:729-730` run UNCONDITIONALLY before the `if is_all_zero:` block at `:737` where the short-circuit lives. The lookups had already executed by the time the short-circuit fired. The plan asserted a guarantee the prescribed code did not deliver; the visible behavior (FEE gone from the reconciliation table) looked correct, masking the false rationale.

**See also:** coding_guidelines.md #25 (Family H parent: verify the real thing, not the abstraction), #17-#24 (Family A-H root-cause catalog), `plans` skill "Investigation Quality Requirements" (verification-first task ordering; code inspection is INSUFFICIENT), `plans` skill Plan Quality Gate (the review sub-agent empirically verifies claims against source - the backstop that catches this family when authoring did not), #190 (review-orchestration Family-H analog: trust the panel-ran label as proof of coverage masks the case where it never launched), AGENTS.md "Code inspection is INSUFFICIENT" and "Before deleting or overwriting, look at the target."


## 192. A RED Test Whose Fixture Contradicts Its Own Stated Intent Is Invisible at RED and Mimics an Implementation Defect at GREEN; When a Spec-Faithful GREEN Still Fails a RED Test, Suspect Fixture/Intent Consistency Before the Implementation

**Principle:** Family H (Verify the real thing, not the abstraction) cross with the TDD-process family of #76/#109/#116. The RED phase validates only ONE property: "the behavior under test is absent." It does NOT validate that the test's fixture constructs the case the the test's docstring names. A fixture that contradicts the test's own stated intent is therefore a DORMANT defect at RED - the test fails for the right reason (behavior absent) regardless of whether the fixture matches the intent, so the standard "RED must fail" check passes the contradiction silently. The contradiction only becomes executable at GREEN, when the production code makes the assertion real, and at that point the failure mimics an implementation defect: the implementer sees "test still RED after a spec-faithful implementation" and the natural hypothesis is "the implementation is wrong," not "the fixture constructs the wrong case."


**Trigger:** A multi-task TDD plan where Task N writes a RED test (the assertion encodes a behavioral contract Task N+1 will satisfy) and Task N+1 flips it GREEN. The GREEN implementation is verified to match the plan/spec pseudocode byte-for-byte, yet the RED test still fails. The implement sub-agent's first hypothesis is an implementation defect; the orchestrator (or a re-read) must diagnose that the test's fixture constructs a DIFFERENT case than the test's docstring describes (e.g. the docstring says "every row is X" but the fixture builds a mix of X and non-X rows). Critically: this is NOT triggered by a contract change between RED and GREEN (that is #109), NOT by a placeholder value colliding with an external new semantic (that is repo-style #61 / user #143), and NOT by a degenerate filler violating an orthogonal production invariant (that is #116).

**Rule:**
1. When a GREEN implementation that matches the plan/spec pseudocode byte-for-byte still fails a RED test whose RED failure was clean (a clean assertion or `pytest.fail`, not a collection/runtime error per #159), the FIRST hypothesis to check is fixture/intent consistency, NOT an implementation defect. Re-read the RED test's fixture against its OWN docstring (without consulting the implementation) and ask: does the fixture actually construct the case the docstring names? RED did not validate this; RED only proved the behavior was absent.
2. The diagnostic for "fixture constructs the wrong case": enumerate every value the fixture sets (or leaves at its default) and trace whether that value routes the fixture into the docstring's named case or into a sibling case. Defaults are the stealth vector - a fixture that omits a classification/flag/enum override inherits the default, which may route the row into a different bucket than the docstring implies. In Task 4's case: the docstring named the "all-dust" case (`not real_rows and dust_rows`), but the fixture left the priced rows at their default classification, which routed them into `taxable_now_entries` and thence into `real_rows`, so the fixture actually constructed the MIXED case and the all-dust branch never fired.
3. The fix is to correct the FIXTURE so it constructs the case the docstring names (in Task 4: reclassify the priced rows so they stay in `reward_entries` as discriminator evidence without entering `taxable_now_entries`). Do NOT contort the implementation to match a misconstructed fixture, and do NOT weaken or delete the assertion. The docstring's stated intent is authoritative; the fixture is the bug.
4. Authoring-time prevention: when writing a RED test whose deliverable is the test itself (committed RED, later-task GREEN per #159), re-read the fixture against the docstring ONE more time before committing the RED task. Ask: "if the implementation matched the spec exactly, would this fixture cause the docstring's named case to fire?" If the answer is no, the RED will pass the "must fail" check for the wrong reason (behavior absent) and the contradiction will surface only at GREEN, where it is more expensive to diagnose because the implementation now looks suspect.
...
**Distinguishing from #116 (boundary-filler violates orthogonal invariant):** #116 is a BOUNDARY/LIMIT characterization test where the fixture author reaches for a DEGENERATE MECHANICAL FILLER (`b"x" * N`, `[None] * M`) to hit an exact size/count/length boundary, and the filler violates an ORTHOGONAL PRODUCTION INVARIANT (parseability, schema validity, non-emptiness). The failure is "the implementation correctly rejects the invalid input," and the fix is content that satisfies the orthogonal invariant at the exact boundary value. This lesson (#192) is NOT a boundary test: the fixture author writes PLAUSIBLE DOMAIN ROWS (no degenerate filler), every row is individually valid, the production path runs cleanly, and the failure is "the fixture constructs a DIFFERENT CASE than the docstring names" (mixed vs all-dust), not "the fixture is invalid for the production path." #116's contradiction is filler-content-vs-orthogonal-invariant; #192's contradiction is fixture-case-vs-docstring-intent. Both surface at GREEN after a spec-faithful implementation, but the fixture shape (degenerate filler vs plausible rows) and the contradiction axis (invariant violation vs wrong-case routing) differ.

**Distinguishing from #109 (re-read RED against revised invariants):** #109 fires when the plan/spec was REVISED between RED and GREEN, so the RED test asserts a contract the revision changed; the fix is to update the assertion to the new contract. This lesson (#192) fires when NOTHING changed between RED and GREEN - the implementation matched the spec pseudocode byte-for-byte, no invariant was revised. The defect was present at RED authoring time (the fixture never matched the intent); it was just invisible because RED does not check fixture/intent consistency.

**Distinguishing from the repo-style #61 / user #143 re-scope family:** those fire when a PRE-EXISTING test (or a deferred test) used a value as an ORTHOGONAL PLACEHOLDER, and a LATER task's behavior change (or contract change) assigns that value a new semantic that breaks the test. The fix is to re-scope the placeholder value. This lesson (#192) fires on a NEW test written in the SAME plan's RED task, where the fixture is internally inconsistent with the test's OWN intent - no external semantic change or contract change is involved, and the fix is to correct the fixture (not re-scope a placeholder).

**Example (2026-07-18 tax-reporting `2026-07-18-crypto-dust-partition-fee-skip` plan, Task 4 GREEN):** Task 3 (RED) wrote `TestCryptoSupplementarySheetDustSummary::test_all_dust_empty_label` with docstring intent "given a `CryptoTaxReport` where EVERY taxable-now row is dust (all assets have a priced row elsewhere), expects the Section 2 detail table shows the all-dust empty label and a dust summary block renders below it." The fixture built four rows - `btc_priced` (value_eur=1), `btc_zero` (value_eur=0), `eth_priced` (value_eur=2), `eth_zero` (value_eur=0) - ALL defaulting to `RewardTaxClassification.TAXABLE_NOW` (no `classification=` override on any row). The two priced rows are non-zero, so under the plan's three-state partition they are `real_rows`, NOT dust. The fixture therefore constructed the MIXED case (2 priced detail rows + 2 dust rows), not the all-dust case the docstring named. At RED this was invisible: the production helpers `_partition_taxable_now` and `_write_dust_summary_block` did not exist yet, so the test failed for the right reason (behavior absent) and passed the standard RED check. At GREEN (Task 4), the implementation matched the plan pseudocode byte-for-byte, but the test still failed because the all-dust branch (`not real_rows and dust_rows`) never fired - `real_rows` held the two priced rows. The GREEN sub-agent initially reported this as a BLOCKING CONTRADICTION and was unsure whether the implementation or the test was wrong; the orchestrator diagnosed it as a Task 3 fixture bug (the fixture contradicted the test's own stated intent) and fixed the fixture by reclassifying the two priced rows to `DEFERRED_BY_LAW`, so they remain in `reward_entries` as priced-row discriminator evidence (proving BTC/ETH are priced assets) without entering `taxable_now_entries` (which would make them non-dust real_rows). After the fixture correction, all 12 Task 3 tests went GREEN and the full module passed 46/46. See the Task 4 implement log "Blocking contradiction (Task 3 RED test fixture vs. plan intent)" and "Orchestrator resolution (Task 3 fixture bug; plan-related extension)" sections.

**See also:** coding_guidelines.md #25 (Family H parent), #76 (TDD RED-then-GREEN as a process step - the transient case), #109 (re-read RED assertions against REVISED invariants before the GREEN flip - the contract-change case), #116 (boundary-filler violates orthogonal invariant - the degenerate-filler case), #143 (re-scope when a fixture flips an orthogonal signal - the migration case), #159 (a RED test that is itself the deliverable must fail as a clean assertion - failure-shape discipline), #191 (a plan that asserts current-code behavior must verify it empirically - the plan-authoring sibling), #193 (substitute an e2e-realizable analog INPUT when middleware routes the literal input away from the code under test - the e2e analog-of-the-input case), CLAUDE.md §4 Agent Workflow Rules (RED-then-GREEN TDD discipline).


## 193. When an E2E Fixture's Literal Input Cannot Reach the Code Under Test Because Upstream Middleware Routes It Elsewhere, Substitute an E2E-Realizable Analog Input That Exercises the Same Discriminator (Justified by Discriminator-Invariance)
...
**Trigger:** An e2e test (or a fixture promoted from a unit fixture to e2e) is meant to pin a behavior of a downstream helper whose discriminator is input-agnostic. The motivating unit fixture used a literal input that the unit test bypassed the middleware to construct directly (e.g. `classification=RewardTaxClassification.TAXABLE_NOW` passed verbatim, skipping the production classifier). The e2e test must run the full pipeline (parse → classify → route → render), so the middleware runs and routes the literal input into a different bucket (e.g. a crypto-denominated ticker routed to `DEFERRED_BY_LAW` rather than `TAXABLE_NOW`), so the downstream helper never sees it and its branch is unexercisable. The implementer notices either (a) the e2e test cannot go GREEN with the literal input no matter how correct the implementation is, or (b) the helper-under-test is silently never reached and the test passes for the wrong reason (testing nothing about the discriminator).
...
5. Re-scope any fixture-adjacent side constraints the substitution introduces. If the analog has a different side property than the literal (here: a zero-value reward on a fiat asset may be retained-or-dropped by a different retention rule than a zero-value reward on a crypto asset), audit that side property and add the minimal supporting fixture row to make the analog survive to the helper-under-test. Do NOT let the side property silently drop the analog before it reaches the discriminator.

**Why this happens:** Unit-tier fixtures are constructed by calling the helper or the dataclass constructor directly, with the discriminator-set field passed verbatim (`classification=TAXABLE_NOW`). This is correct for a unit test of the helper, but it silently depends on the assumption that the production middleware would have produced the same field value. When the same motivating input is later promoted to an e2e fixture (to pin the full pipeline), the middleware runs, and the assumption is exposed: the middleware may route the literal input elsewhere because its bucket rule is orthogonal to the helper's discriminator. The plan author wrote the e2e task's prose naming the literal input (mirroring the unit fixture) without re-tracing whether the literal input survives the middleware, because the unit fixture never ran the middleware. The mismatch surfaces only at e2e authoring time.

**Shape trigger (when to suspect this family):**
- An e2e fixture's motivating prose (or its unit-fixture predecessor) names a specific ticker/identifier/value, and the e2e test must run the full pipeline (parse → classify → route → render), not a direct helper call.
- The production pipeline has a classifier/normalizer/router between parse and the code under test whose bucket rule is ORTHOGONAL to the discriminator the code under test reads (e.g. classifier branches on "is the ticker in the fiat set" while the helper branches on "is value_eur > 0").
- The literal motivating input is a member of a bucket the classifier routes AWAY from the code under test (e.g. a crypto-denominated ticker that the classifier routes to DEFERRED_BY_LAW, while the helper operates only on TAXABLE_NOW entries).
...
**Distinguishing from #20 (verify path reachability before writing a test; mock OR amend):** #20 lists two responses when a path is unreachable via real data - (a) mock/patch to inject the edge case, or (b) amend the implementation to make the path reachable. This lesson (#193) is the e2e-specific THIRD option: substitute an e2e-realizable analog INPUT so the path becomes reachable through the REAL pipeline without mocking and without amending. #20's "unreachable" is about a defensive/guard path that real data never exercises (placeholder mechanism always fires first); #193's "unreachable" is about a downstream helper that real data WOULD exercise on an equivalence class, but the literal motivating input happens not to be a member of that class under the real middleware. #20 accepts mocking as a valid response (it is about a unit-tier guard test); #193 rejects mocking (it is about an e2e test whose purpose is the real pipeline).

**Distinguishing from #114 (re-scope the assertion when synthetic identifiers flip orthogonal signals):** #114 fires when migrating a test off a real fixture to synthetic data, where the synthetic identifiers flip orthogonal DOWNSTREAM SIGNALS (review flags, sentinel values) that the OLD ASSERTION checked as side properties; the fix is to re-scope the ASSERTION to the primary behavior. #193 fires when the literal INPUT cannot reach the code under test at all; the fix is to substitute the INPUT, not the assertion. #114 is about the assertion (keep the fixture, change what you assert); #193 is about the fixture input (keep the assertion, change what you feed in). They compose but address different layers: a single migration could hit both - first substitute the input (#193) so the helper is reachable, then re-scope any assertion the analog's side properties flipped (#114).

**Example (2026-07-18 tax-reporting `2026-07-18-crypto-dust-partition-fee-skip` plan, Task 7 e2e):** The plan's Task 7 prose named `BTC` (priced) and `OSBGT` (unpriced) as the motivating tickers for the dust-partition e2e fixture, mirroring the unit fixtures in `TestCryptoSupplementarySheetDustSummary`. The unit tests construct `CryptoRewardIncomeEntry` rows with `classification=RewardTaxClassification.TAXABLE_NOW` passed verbatim, bypassing the production classifier. The e2e fixture (`resources/source/example/2025/koinly/dust-partition/`) must run the full Koinly → workbook → dust-block pipeline, so `_classify_reward_tax_status` runs and routes EVERY crypto-denominated ticker (BTC, OSBGT, BGT, USDT) to `DEFERRED_BY_LAW`, because its TAXABLE_NOW branch is defined as "asset is in `_get_all_fiat_currency_codes()` (ISO 4217 minus carve-outs)". The dust partition operates exclusively on `taxable_now_entries`, so with BTC/OSBGT the partition would see an empty list and the dust branch would be unexercisable. The fixture substitutes the fiat codes `EUR` (priced analog of BTC) and `USD` (unpriced analog of OSBGT). The substitution is faithful because `_partition_taxable_now`'s discriminator is `e.asset in priced_assets_in_export` where `priced_assets_in_export = {e.asset for e in reward_entries if e.value_eur > 0}` - the discriminator reads `value_eur > 0` and `asset` identity, both of which EUR/BTC and USD/OSBGT produce identically (a priced EUR row makes EUR priced just as a priced BTC row would make BTC priced; a zero USD row with no priced USD row is unpriced just as a zero OSBGT row would be). The fixture also adds a priced USD CG row to keep USD in `known_assets` (a side constraint - the zero-value USD income reward would otherwise be dropped by the `is_known` retention rule at `crypto_reporting.py:982-995` before it reaches the partition; this is the "re-scope side constraints" step). The test docstring documents the BTC→EUR, OSBGT→USD substitution per #114 (the assertion-scope cousin). All three e2e test methods go GREEN exercising the real pipeline. See the Task 7 implement log "Plan-vs-fixture asset substitution (BTC → EUR, OSBGT → USD)" and "Reward classifier constraint" sections.

**See also:** coding_guidelines.md #25 (Family H parent), #20 (path reachability - mock OR amend; #193 is the e2e third option), #114 (re-scope the assertion when synthetic identifiers flip orthogonal signals - the assertion-layer cousin; #193 is the input-layer cousin), #143 (re-scope placeholder values when a later task assigns them a new semantic), #192 (RED fixture contradicts own stated intent - a fixture-construction defect, not a fixture-input substitution), #194 (a RED test pinning a NEGATIVE contract is legitimately GREEN at RED when production already satisfies it; the negative-assertion vertex of the RED-phase cluster), CLAUDE.md §4 Agent Workflow Rules.


## 194. A RED Test Pinning a NEGATIVE Contract (Must-NOT-Render on an Empty Precondition) Is Legitimately GREEN at RED When Production Already Satisfies It; Do Not Force It RED to Hit a "All N Tests Fail" Quota

**Principle:** Family H (Verify the real thing, not the abstraction) cross with the TDD-process family of #76/#109/#159/#192/#193. The RED phase of a TDD plan has TWO distinct contract shapes: (1) a POSITIVE contract ("the GREEN task adds behavior X, which is absent today, so the RED test fails now"), and (2) a NEGATIVE contract ("the GREEN task preserves behavior Y on an empty precondition, which is ALREADY satisfied today because the block is conditionally rendered"). A RED test for a negative contract is, by construction, a REGRESSION GUARD against a future always-render regression; it cannot go RED at authoring time because the production code already satisfies the negative contract. Forcing it to fail (via `pytest.fail`, a sentinel, or a `try/except` scaffold) to hit an "all N tests fail RED" summary quota is a category error: it converts a regression guard into a transient process step, destroying its guard value and conflating it with the positive-contract behavioral tests it sits beside.
...
**Distinguishing from #192 (RED fixture contradicts own stated intent):** #192 fires when a RED test's FIXTURE constructs the wrong case (mixed vs all-dust), so the docstring's named case never fires and the test fails for the wrong reason at GREEN. This lesson (#194) fires when the RED test's CONTRACT is negative (must-not-render) and production already satisfies it, so the test PASSES at RED by design. #192's contradiction is fixture-vs-intent (a defect); #194's pass is contract-shape-correct (a feature). #192 bites at GREEN (the implementation runs, the wrong case fires, the test fails for the wrong reason); #194 is observable at RED (the test passes when the summary says all should fail).

**Distinguishing from #159 (committed-RED must fail as a clean assertion):** #159 prescribes the FAILURE SHAPE of a RED test whose deliverable is the committed RED (clean `pytest.fail` naming the resolving task, never an unhandled exception). This lesson (#194) prescribes the PASS-vs-FAIL CLASSIFICATION of a RED test whose contract is negative: it is legitimately GREEN at RED, and forcing it to fail (via `pytest.fail` or a scaffold) violates #159 in the opposite direction by introducing a failure mode the GREEN task must then delete. #159 is about how a DESIGNED-RED test fails; #194 is about recognizing when a test is NOT designed-RED at all because its contract is negative and already satisfied.

**Distinguishing from #46/#133 (regression test must exercise the guarded production path; disable-and-confirm-RED):** #46/#133 prescribe the GREEN-time verification that a guard-binding test actually binds the guarded path (disable the guard, confirm the test fails). This lesson (#194) prescribes the RED-time classification that recognizes a negative-contract test as a guard, not a behavioral RED. The two compose: at RED, classify the contract shape (#194); at GREEN, run the disable-and-confirm-RED check (#46/#133) to verify the negative-contract test actually binds the conditional render.

**Distinguishing from #109 (re-read RED against revised invariants):** #109 fires when the plan was REVISED between RED and GREEN, so the RED test's assertions are stale relative to the new contract. This lesson (#194) fires when NOTHING was revised: the negative-contract test pins a contract the production code already satisfies, and the test passes at RED by design. #109 is about assertion freshness under revision; #194 is about contract-shape classification at authoring time.

**Example (2026-07-19 tax-reporting `2026-07-19-deferred-reward-dust-skip` plan, Task 3 RED):** Task 3's deliverable was the `TestCryptoSupplementarySheetDeferredSkip` class with 7 tests pinning the Section 3 "Suppressed zero-value deferred rewards" block that Task 4 will render. Six tests pin POSITIVE contracts (the block renders with the dust reason, the unpriced reason, the merged single-block sorted order, the `:.8f` amount format, the per-(asset,wallet) sort, the `safe_cell_value` wrappers); these fail RED naturally because the block is absent today (header count `0 != 1`). The seventh test, `test_empty_skipped_list_renders_no_block`, pins a NEGATIVE contract: given an empty `skipped_zero_value_deferred_rewards` list, the rendered output must NOT contain the "Suppressed zero-value deferred rewards" header. The verbatim Task 3 spec described this test as a "regression guard on the conditional render." At the RED run, 6 tests failed and `test_empty_skipped_list_renders_no_block` PASSED, because production already conditionally renders (the block does not exist yet, so it is trivially absent on the empty precondition). The implement log documented the asymmetry under "Deviations from plan": cited the verbatim "regression guard on the conditional render" wording, cited the Part 7 sibling precedent `test_no_rewards_empty_label_unchanged` (line 1138 of the same test file, which also passes in its RED phase for the same reason), and stated that the 6 behavioral tests fail RED via natural assertion failure while the negative-contract test passes as a regression guard. The test was NOT forced RED via `pytest.fail` or a scaffold; it remained a clean `assert header_count == 0` assertion whose GREEN-today state is the natural result of the current conditional render. The full module run confirmed `6 failed, 48 passed` - all 48 pre-existing tests stayed GREEN and the only failures were the 6 new positive-contract behavioral RED tests. See the Task 3 implement log "Deviations from plan" section and the plan `docs/history/plans/2026-07-19-deferred-reward-dust-skip.md` Task 3 verbatim spec.

**See also:** coding_guidelines.md #25 (Family H parent), #76 (TDD RED-then-GREEN as a process step - the positive-contract transient case), #109 (re-read RED against revised invariants - the contract-change case, not the contract-shape case), #133/#46 (regression test must exercise the guarded production path; disable-and-confirm-RED - the GREEN-time guard-binding verification that pairs with this RED-time classification), #159 (committed-RED must fail as a clean assertion - the failure-shape discipline this lesson extends to the negative-contract vertex by forbidding forced failure), #192 (RED fixture contradicts own stated intent - a fixture defect at GREEN, not a contract-shape classification at RED), #193 (e2e-realizable analog input substitution - the input-layer cousin in the same plan family), CLAUDE.md §4 Agent Workflow Rules (RED-then-GREEN TDD discipline).

## 195. When You Expand an Open PR's Scope (Files and Body), Update the Title in the Same Turn
...
## 196. Keep Code-Structure Keys Out of Per-Workspace Config-Server Overlays
...
**See also:** #200 (verify overlay `spring.application.name` against the target JAR), project `operational-guides.md` (MyBatis type-handler scanning), CRM naming ADR for deployable vs runtime identity (related but distinct: filename vs `spring.application.name`).

## 197. Read the Wiki Page Body Before Citing It for a Control Claim
...
**See also:** #10 (descriptive labels), #197 (do not invent controls from unopened wiki pages).


## 199. Shared Skill Symlink: Install CLIs Must Not Rewrite the Playbook Tree
...
## 200. Config-Server Overlay Identity Keys Must Match the Target Service JAR
...
**See also:** #196 (code-structure keys out of overlays), #195 (PR title stays in sync with ticket/branch), doing-code-review §4.2 assumption checks.


## 201. When Inserting a Test Class Into an Existing File, Grep for Module-Level Helpers First (F811 May Be Off)
...
**Example (2026-07-24 crypto-logging-review r2, Low):** The derivatives-filter review inserted `class TestEmitters` into `tests/unit/application/test_derivatives_filter.py` along with a helper block re-declaring `_TEST_OPERATOR_ORIGIN` and `_make_cg_lot`. The originals already existed at lines 639 and 652; the new copies landed at lines 956 and 969, byte-identical. Python silently rebound both (all 683 affected tests passed), and `uv run ruff check` reported clean because F811 is configured off in this repo. Round 1 missed it (green + clean lint). Round 2's fresh diff-hunk read found the duplication. The fix removed the second copies (old lines 956-1004), keeping the canonical originals. Full suite 1852 green after removal.

**Distinguishing from #1:** Lesson #1 is the thin Family D seed ("always check for duplicate test methods or functions before adding new code", `grep -n "def method_name" -r`). This lesson #201 is the specific, non-obvious aggravator that makes #1 actually fire reliably: F811 is OFF here, so "ruff clean" cannot substitute for the grep, and byte-identical module-level helper duplicates are the exact shape that is invisible to both the test suite and the configured linter. #1 says "grep first"; #201 says "grep first AND do not trust ruff-clean, because F811 is off."

**Distinguishing from #95:** Lesson #95 is about `ruff check --fix` REMOVING intended re-export imports (F401 false-positive on backward-compat re-export modules). This lesson #201 is the opposite direction: ruff FAILING to flag an UNINTENDED module-level redefinition (F811 off). #95 = ruff over-deletes; #201 = ruff under-reports. Both are "ruff config gap" lessons but on different rules (F401 vs F811) with opposite failure modes.
...
**See also:** Lesson #1 (duplicate-detection Family D seed), Lesson #95 (do not `ruff --fix` on re-export modules), CLAUDE.md "Code Quality" (Ruff primary linter/formatter).

## 202. Do Not Override an Agent-Enforced Mechanical Gate by Restoring Policy-Violating Content
